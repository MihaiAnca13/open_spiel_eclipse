...
class PPO(nn.Module):
  ...
  @staticmethod
  def _select_obs_rows(main, extra, n_main, sel):
    """Obs rows for logical ids ``sel``, drawn from ``main`` and/or ``extra``.

    Distinct from ``_gather_obs``, which pulls the current per-seat observation
    out of a TimeStep -- do not merge the two names again.

    ``main`` is a view of the CPU rollout buffer and ``extra`` holds the terminal
    closeout samples. Only the selected rows are materialised, so the caller pays
    one minibatch of memory instead of a copy of the whole buffer.
    """
    if extra is None or extra.shape[0] == 0:
      return main[sel]
    from_main = sel < n_main
    if bool(from_main.all()):
      return main[sel]
    if not bool(from_main.any()):
      return extra[sel - n_main]
    out = torch.empty((sel.shape[0],) + tuple(main.shape[1:]), dtype=main.dtype)
    out[from_main] = main[sel[from_main]]
    out[~from_main] = extra[sel[~from_main] - n_main]
    return out
  ...
  def _learn_core(self, next_obs):
    ...
    b_legal_actions_mask = self.legal_actions_mask.reshape(
        (-1, self.num_actions))
    # A VIEW of the CPU rollout buffer -- never copied wholesale. Logical batch
    # rows are addressed through `obs_rows` (built below), which indexes this
    # view for ids < n_obs_main and the closeout extras beyond that. Filtering
    # or concatenating this tensor directly costs a full buffer copy.
    b_obs = self.obs.reshape((-1,) + self.input_shape)
    n_obs_main = b_obs.shape[0]
    ex_obs = None
    ...
    if n_extra:
      ...
      ex_targets = torch.tensor([e[7]
                                 for e in self._extra_samples]).to(self.device)
      # NOT torch.cat: b_obs is a view of the whole CPU rollout buffer, so
      # concatenating a handful of closeout rows onto it copied the entire
      # buffer (4.9 GB at 256 envs x 128 steps) on EVERY learn call. The extras
      # stay separate and `obs_rows` below addresses both sources, so only the
      # minibatch is ever materialised.
      if not use_sparse:
    ...
    # Logical row -> obs source row. ids < n_obs_main index the CPU rollout
    # buffer view; ids beyond it index `ex_obs` at (id - n_obs_main). Slicing
    # THIS (260 KB of int64) is what replaces copying 4.9 GB of observations.
    obs_rows = torch.arange(n_obs_main + n_extra, dtype=torch.long)
    ...
    if self.league:
      ...
      # indexed the CPU rollout buffer with a CUDA mask ("indices should be
      # either on cpu or on the same device"), which broke --league outright
      # from the commit that moved the buffer off the GPU; and even when it
      # worked it copied the whole buffer a second time.
      obs_rows = obs_rows[torch.from_numpy(keep_np)]
      if not use_sparse:
    ...
    batch_size = (obs_rows.numel() // self.num_minibatches) * self.num_minibatches
    if batch_size != obs_rows.numel():
      obs_rows = obs_rows[:batch_size]
    ...
    for _ in range(self.update_epochs):
      ...
      for start in range(0, batch_size, minibatch_size):
        ...
        mb_obs = self._select_obs_rows(b_obs, ex_obs, n_obs_main,
                                       obs_rows[mb_inds]).to(self.device)
